...
describe('ClassroomDetailPage instructor materials', () => {
  it('renders the classroom resource table in week order and uploads a dropped PDF', async () => {
    ...
    const uploadedResourceButton = screen.getByRole('button', { name: '수업 참고자료' })
    expect(uploadedResourceButton.parentElement?.firstElementChild).toHaveTextContent('자료')
    ...
    expect(within(contentRegion).getByText('수업')).toHaveClass('min-h-5', 'rounded-md', 'border', 'px-1.5', 'type-caption', 'font-semibold')
    const lectureButton = screen.getByText('lecture').closest('button')
    expect(lectureButton?.parentElement).toHaveClass('h-11', 'min-h-11')
    expect(within(lectureButton!).queryByText('수업', { exact: true })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: '수업' }))
    expect(screen.getByRole('button', { name: 'lecture' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '수업 참고자료' })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: '자료' }))
    expect(screen.getByRole('button', { name: '수업 참고자료' })).toBeInTheDocument()
