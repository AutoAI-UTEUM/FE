...
function InstructorExamView({ exam, isWorking, onAction, onUpdated, repository }: { exam: Exam; isWorking: boolean; onAction: (action: 'publish' | 'close' | 'delete') => void; onUpdated: (exam: Exam) => void; repository: ReturnType<typeof createExamsRepository> }) {
  ...
  return <>
    ...
    {exam.status !== 'DRAFT' ? <section className="overflow-hidden rounded-xl border border-stone-200 bg-white"><div className="border-b border-stone-200 px-5 py-4"><h2 className="type-section-title font-bold">제출 현황</h2></div>{submissionsError ? <p className="border-b border-stone-100 px-5 py-3 type-control text-rose-700" role="alert">{submissionsError}</p> : null}{submissions.length === 0 ? <EmptyState title="제출 내역이 없습니다" description="학습자가 시험을 제출하면 여기에 표시됩니다." /> : <div>
      {/* 780px 가로 스크롤 표 대신, 다른 목록 화면과 같은 반응형 그리드를 쓴다. 폰에서는 한 건이 카드 한 장이 된다. */}
      <div className={SUBMISSION_GRID + ' hidden min-h-10 items-center gap-3 border-b border-stone-200 bg-stone-50 px-5 type-caption font-semibold text-stone-500 lg:grid'}><span>학습자</span><span>상태</span><span>시도</span><span>점수</span><span>제출 시각</span><span className="text-right">작업</span></div>
      {submissions.map((submission) => <article className={'grid gap-2 border-b border-stone-100 px-5 py-3 type-control last:border-0 lg:items-center lg:gap-3 ' + SUBMISSION_GRID} key={submission.id}>
        <strong className="truncate font-semibold text-stone-900">{submission.userName}</strong>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 type-caption text-stone-500 lg:contents">
          <span className="lg:type-control lg:text-stone-700">{getSubmissionLabel(submission.status)}</span>
          <span className="lg:type-control lg:text-stone-700"><span className="text-stone-400 lg:hidden">시도 </span>{submission.attemptNo}/{submission.attemptCount}</span>
          <span className="lg:type-control lg:text-stone-700"><span className="text-stone-400 lg:hidden">점수 </span>{submission.status === 'GRADED' ? `${submission.score ?? 0}/${submission.maxScore ?? 0}` : '-'}</span>
          <span className="lg:type-control">{formatDateTime(submission.submittedAt)}</span>
        </div>
        {submission.status === 'GRADING_FAILED' ? <div className="lg:justify-self-end"><Button disabled={regradingSubmissionId !== null} onClick={() => void regradeSubmission(submission.id)} size="sm" variant="secondary">{regradingSubmissionId === submission.id ? '요청 중' : '재채점'}</Button></div> : <span className="hidden lg:block" />}
      </article>)}
    </div>}</section> : null}
...
const SUBMISSION_GRID = 'lg:grid-cols-[minmax(160px,1.4fr)_92px_72px_92px_minmax(140px,1fr)_96px]'
