import { Copy, Download, KeyRound, MoreHorizontal, Search, UserRoundX } from 'lucide-react'
...
import { Button, ButtonLink, EmptyState, PageToolbar, Select, useToast } from '../../../shared/ui'
  ...
  return (
    <ClassroomWorkspaceContainer>
      <ClassroomWorkspaceHeader
        actions={<><Button onClick={() => void copyInviteCode()} variant="secondary"><KeyRound aria-hidden="true" size={14} /><span className="type-invite-code">{classroom.inviteCode ?? '초대 코드'}</span><Copy aria-hidden="true" size={13} /></Button><Button disabled={students.length === 0} onClick={exportRoster} variant="secondary"><Download aria-hidden="true" size={14} />명단 내보내기</Button></>}
        activeTab="learning"
        classroom={classroom}
      />

      <PageToolbar>
        <label className="w-full min-[520px]:w-auto"><span className="sr-only">수강생 정렬</span><Select aria-label="수강생 정렬" className="w-full min-w-36 min-[520px]:w-auto" onChange={(event) => setSort(event.target.value as ClassroomStudentSort)} value={sort}><option value="RECENT_ACTIVITY">최근 활동순</option><option value="NAME">이름순</option><option value="LOW_PROGRESS">낮은 진도순</option></Select></label>
        ...
        <label className="relative ml-auto min-w-48 sm:w-72 mobile-phone:ml-0 mobile-phone:min-w-full"><span className="sr-only">수강생 검색</span><Search aria-hidden="true" className="absolute top-1/2 left-3 -translate-y-1/2 text-stone-400" size={14} /><input className="h-10 w-full rounded-lg border border-stone-200 bg-white pr-3 pl-9 type-control outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 mobile-web:h-11" onChange={(event) => setQuery(event.target.value)} placeholder="이름 검색" value={query} /></label>
      </PageToolbar>
