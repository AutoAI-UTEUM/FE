  ...
  it('shows xAI credits, risk and performs a manual sync', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url.pathname === '/api/admin/xai/overview' || (url.pathname === '/api/admin/xai/sync' && method === 'POST')) {
        return success({ available: true, averageDailyCost7d: '4.85', costSource: 'INTERNAL', currentMonthCostUsd: '72.75', fetchedAt: '2026-09-20T08:00:00Z', lastSuccessfulSyncAt: '2026-09-20T08:00:00Z', postpaidLimitUsd: '300.00', postpaidRemainingUsd: '227.25', postpaidUsedUsd: '72.75', prepaidAvailableUsd: '125.00', prepaidBalanceUsd: '140.00', prepaidUsedThisPeriodUsd: '15.00', projectedDepletionAt: '2026-12-01T08:00:00Z', riskLevel: 'WARNING', stale: false, totalAvailableUsd: '352.25' })
      }
    ...
    // 선불 표시는 원장값(prepaidBalanceUsd $140.00)이 아니라 가용액(prepaidAvailableUsd $125.00)이어야 한다.
    expect(screen.getByText(/선불 잔액 \$125\.00/)).toBeInTheDocument()
