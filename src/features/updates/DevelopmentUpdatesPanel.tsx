...
export function DevelopmentUpdatesPanel({ initialDate, repository, showTitle = true }: { initialDate?: Date; repository?: UpdatesRepository; showTitle?: boolean }) {
  ...
  return (
    <section aria-label={showTitle ? undefined : '업데이트'} aria-labelledby={showTitle ? 'development-updates-title' : undefined} className="mx-auto flex h-full min-h-0 w-full max-w-[1560px] flex-col gap-[14px] overflow-y-auto pb-1" ref={measureArea}>
      ...
      <div className="grid shrink-0 grid-cols-1 gap-[14px] sm:grid-cols-2 xl:grid-cols-4">
        <UpdateSummaryCard dark detail={latestUpdate ? `최근 배포 ${formatCompactDate(latestUpdate.date)}` : '배포 기록 없음'} label={`${visibleMonth.getMonth() + 1}월 배포`} suffix="건" value={updates.length} />
        <UpdateSummaryCard detail={`전체의 ${formatShare(feCount, updates.length)}`} label="FE" suffix="건" value={feCount} />
        <UpdateSummaryCard detail={`전체의 ${formatShare(aiBeCount, updates.length)}`} label="AI · BE" suffix="건" value={aiBeCount} />
        <UpdateSummaryCard detail={`${visibleMonth.getMonth() + 1}월 ${daysInMonth}일 중`} label="배포한 날" suffix="일" value={activeDays} />
      </div>

      <div className="grid shrink-0 items-stretch gap-[14px] xl:grid-cols-2">
      <section className="h-[560px] min-w-0 overflow-hidden rounded-[14px] border border-stone-200 bg-white px-[22px] py-5 mobile-phone:h-auto" aria-label={`${monthLabel} 업데이트 달력`}>
        <div aria-label="업데이트 도구" className={cx('flex flex-wrap items-center justify-between gap-3', areaWidth < 700 && 'justify-center')} role="toolbar">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <strong className="type-control font-bold text-stone-900">{monthLabel}</strong>
            <div className="ml-1 flex items-center gap-1">
              <button aria-label="이전 달" className="flex size-8 items-center justify-center rounded-lg text-stone-400 hover:bg-stone-50 hover:text-stone-800 mobile-web:size-11" onClick={() => moveMonth(-1)} type="button"><ChevronLeft aria-hidden="true" size={15} /></button>
              <button aria-label="다음 달" className="flex size-8 items-center justify-center rounded-lg text-stone-400 hover:bg-stone-50 hover:text-stone-800 mobile-web:size-11" onClick={() => moveMonth(1)} type="button"><ChevronRight aria-hidden="true" size={15} /></button>
            </div>
          </div>
      ...
      <aside aria-label="월별 업데이트 목록" className="flex h-[560px] min-h-0 min-w-0 flex-col overflow-hidden rounded-[14px] border border-stone-200 bg-white px-[22px] py-5 mobile-phone:h-auto mobile-phone:min-h-80">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-baseline gap-2"><h3 aria-label={activeDate.label} className="type-section-title font-bold text-stone-950">{activeDate.label} 배포</h3><span className="sr-only">{activeDate.weekday}</span></div>
        </div>
...
function UpdateSummaryCard({ dark = false, detail, label, suffix, value }: { dark?: boolean; detail: string; label: string; suffix: string; value: number }) {
  return <article className={`flex min-h-32 min-w-0 flex-col gap-2.5 rounded-[14px] px-5 py-[18px] ${dark ? 'bg-[#1B2436] text-white' : 'border border-stone-200 bg-white text-stone-950'}`}><p className={`type-caption font-medium ${dark ? 'text-[#A9B4C7]' : 'text-stone-500'}`}>{label}</p><strong className="type-metric font-extrabold tracking-normal tabular-nums">{value.toLocaleString('ko-KR')}{suffix}</strong><p className={`mt-auto type-caption ${dark ? 'text-[#8D99AD]' : 'text-stone-400'}`}>{detail}</p></article>
}
