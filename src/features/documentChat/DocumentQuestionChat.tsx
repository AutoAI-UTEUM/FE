    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <form className="shrink-0 border-t border-stone-200 p-3" onSubmit={submit}>
        <div className="flex items-end gap-2 rounded-xl border border-stone-300 bg-white px-3 py-2 focus-within:border-brand-600 focus-within:ring-2 focus-within:ring-brand-100">
          ...
          <button aria-label={isPending ? '응답 대기 중' : '질문 보내기'} className="touch-target flex size-8 shrink-0 items-center justify-center rounded-lg bg-brand-600 text-white disabled:bg-stone-300" disabled={isPending || !question.trim()} type="submit">{isPending ? <LoaderCircle className="animate-spin" size={15} /> : <Send size={15} />}</button>
        </div>
